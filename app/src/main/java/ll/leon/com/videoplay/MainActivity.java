...
import ll.leon.com.videoplay.video.AudioVideoPlayer;
import ll.leon.com.videoplay.video.VideoView;
...
public class MainActivity extends Activity {
    ...
    private void initView() {
        ...
        player =AudioVideoPlayer.getInstance().getMediaPlayer();
    ...
    private void play() {
        ...
        if (AudioVideoPlayer.getInstance().isPause) {//如果是暂停状态下播放，直接start
            AudioVideoPlayer.getInstance().resume();
            return;
        }
//
...
//        }
        ...
        AudioVideoPlayer.getInstance().initPlayer(path, new MediaPlayer.OnPreparedListener() {
            @Override
    ...
    private void pause() {
        AudioVideoPlayer.getInstance().pause();
        play.setEnabled(true);
    ...
    public void replay(View v) {
        AudioVideoPlayer.getInstance().seekTo(0);
    }

    private void stop(){
        AudioVideoPlayer.getInstance().stop();
        if (player != null) {
            sb.setProgress(0);
            if (timer != null) {
                timer.cancel();
    ...
    protected void onDestroy() {
        super.onDestroy();
        AudioVideoPlayer.getInstance().realese();
    }
}
